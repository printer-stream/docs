...
- When using the page mode with models that support this function, the following rules apply. 
   ...
   - If the printable area is changed with this function, after executing ESC L and FS L, set the printing area to the same size as that specified in ESC W with this function. 
...
**For this printer, the horizontal size of the printable area cannot be changed with this command. Always specify [(** wxL **+** wxH × **256) = 65535]. The horizontal size of the printable area is decided by the setting value for** FS ( L **<Function 33> paper layout (** sm **or** sf **). See the** FS ( L **<Function 33> model information for details.** 
