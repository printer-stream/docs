...
**Bit 2: When the peeling issuing mode is selected, this bit is changed during paper feeding or when a label is in the peeling position.  When the continuous issuing mode is selected, this bit is always 1. The peeling issuing mode/continuous issuing mode is selected with the operation shown below. Selecting the peeling issuing mode** 
...
**Use this command according to the following rule; otherwise, the status might not be transmitted. The next data is not transmitted until the status is received when this command is transmitted.** 
