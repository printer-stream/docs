## **C O N F I D E N T I A L GS z 0 (TM-L90 w/ Peeler)** 

SETTING COMMAND 

[Name] Set online recovery wait time 

[Format] ASCII GS z 0 t1 t2 Hex 1D 7A 30 t1 t2 Decimal 29 122 48 t1 t2 [Range] t1 = 0, 0 ≤ t2 ≤ 255 [Default] t1 = 0, t2 = 0 

- [Description] When the peeling issuing mode is selected, sets the online recovery wait time, the wait time for the FEED button to be pressed when the printer goes online (recovery confirmation time), to (t2 × 500 msec). When t2 = 0, the recovery confirmation time is canceled. 

- [Notes] ■ when the peeling issuing mode is selected, the settings of this command affect the online recovery wait status described below. The peeling issuing mode and the continuous issuing mode can be selected by a switch. This is a slide switch that can be used when the roll paper cover is open: 
