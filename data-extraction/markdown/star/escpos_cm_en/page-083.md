...
- x L and xH specify the horizontal direction for one NV bit image (xL + xH x 256) x 8 dots.

- y L and yH specify the vertical direction for one NV bit image (yL + yH x 256) x 8 dots.

Details

- This command erases all previously defined NV bit images.  The printer cannot redefine only one of several data definitions that had been defined before. Therefore, all data must be resent.
...
- When the amount of data exceeds the capacity left in the range defined by x L , xH, y L , yH, the printer processes an argument that is out of the defined range.
