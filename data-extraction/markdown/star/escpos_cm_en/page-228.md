...
This command is for printing 2-dimensional bar code QR codes.  There are four functions of the commands re -lating to the 2-dimensional bar code QR codes, shown below.

| (1) Set bar code type   | (<ESC> <GS> 'y' 'S')   |
|-------------------------|------------------------|
| (2) Set bar code data   | (<ESC> <GS> 'y' 'D')   |
| (3) Set Star page mode  | (<ESC> <GS> 'y' 'T')   |
...
QR codes can be read even if a part of the data is corrupted, by using error correction.  Raising this level in -creases the size of the bar code because there is an increase in preparatory information.

&lt;ESC&gt; &lt;GS&gt; 'y' 'S' '2'  Specifies the size of the cell (One four squared region configuring the QR code) The QR code is formed into a square of an equivalent size in the vertical and horizontal directions, but the size of the bar code image that is generated depends on the cell size setting.  See Appendix 9 for details on the ac -tual printed size of the QR code.
