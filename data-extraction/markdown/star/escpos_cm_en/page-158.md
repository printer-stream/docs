...
- x L and xH specify the horizontal direction data count for one bit image (xL + xH x 256) in bytes.
- y L and yH specify the vertical direction data count for one bit image (yL + yH x 256) in dots.
- This command is effective only when there is no print data in the print buffer when standard mode is selected.
...
- On STAR printers, the ACK pulse width when using a parallel interface is fixed at 1 μsec.
