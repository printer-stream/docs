...
| (   | 28   |   40 | "(" is inserted for the HRI character. It can be used when you want the HRI characters that indicate AI to look nice when used with the pair of ")." ")." does not constitute encoded data.                                         |
|-----|------|------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| )   | 29   |   41 | After d1 , the first ")" is processed as AI and the data part delimiter, and ")" is inserted for the HRI characters. ")" are inserted for the HRI characters for subsequent ")" . In any case, SP does not constitute encoded data. |
| *   | 2A   |   42 | Check digit A (1 character), calculated with modulus 10 is added as the data part at the position of *. "*" is not an HRI character, but check digits are inserted for HRI characters.                                              |
...
Example:  When [AI = 01/data = 9501234567890/Specify to add check digit A/enclose AI in (), and insert a
...
Example:  When linking [AI = 01/data = 9501234567890/Specify to add check digit A/enclose AI in ()] and [AI=3102/data = 000400/enclose AI in ()], and separating the HRI characters between the link with
